...
Commontator.configure do |config|
  ...
  # Returns: the current member (acts_as_commontator)
  # The default works for Devise and similar authentication plugins
  # Default: lambda { |controller| controller.current_member }
  ...
  config.user_link_proc = lambda { |member, app_routes| '' }
  ...
  #              user, 1, :s => 60, :d => 'mm') }
  config.user_avatar_proc = lambda { |member, view|
                                     view.commontator_gravatar_image_tag(
                                       member, 1, :s => 60, :d => 'mm') }
  ...
  config.user_email_proc = lambda { |member, mailer| member.try(:email) || '' }
  ...
  config.thread_read_proc = lambda { |thread, member| true }
  ...
  config.thread_moderator_proc = lambda { |thread, member| false }
  ...
  config.user_mentions_proc = lambda { |current_member, query|
    current_member.class.where('name LIKE ?', "#{query}%") }
end
